...
export default function StudyApplyForm(props: TStudyFormProps) {
  const {
    titlePlaceholder,
    contentPlaceholder,
    titleMaxLength = 30,
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    const result = await insertStudyForm(formData);
    if (result.success) {
      setTitle('');
      setContent('');
      setSelectedFile(null);
  ...
  return (
    <form action={handleSubmit} className="w-full">
      ...
      <div className="flex flex-col items-start">
        ...
        <div className="w-full flex flex-col items-start mb-[4rem]">
          <label
            htmlFor="content"
            className="font-medium text-content-1 mb-[1.2rem] text-gray-900"
          >
